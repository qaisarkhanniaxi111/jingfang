...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    ...
    <input class="form-control" id="name" type="text" name="field-name" placeholder="Enter Name"></div>
    <div class="mb-6">
    <label class="form-label" for="">Threshold</label>
    <input class="form-control" id="threshold" type="number" name="field-name" placeholder="Enter Threshold"></div>
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between">
      <div class="col-12 col-lg-auto mb-4 mb-lg-0">
        <div class="d-flex align-items-center">
          <h4 class="mb-0 me-2">Groups</h4>
          <span class="badge bg-primary rounded-pill small" id="total">{{count($groups)}} Total</span>
        </div>
      </div>
...
<section class="py-8"><div class="container">
  <div class="px-4 pb-4 mb-6 bg-white rounded shadow">
    <div class="table-responsive">
      <table class="table mb-0 table-borderless table-striped small"><thead><tr class="text-secondary"><th class="pt-4 pb-3 px-6">Name</th><th class="pt-4 pb-3 px-6">Threshold</th><th class="pt-4 pb-3 px-6">Action</th></tr></thead><tbody id="tb">
        @foreach($groups as $g)
        <tr id="row{{$g->id}}">
        <td class="py-5 px-6" id="name{{$g->id}}">{{$g->name}}</td>
        <td class="py-5 px-6" id="threshold{{$g->id}}">{{$g->threshold}}</td>
...
$("#actionnew").click(function(){
  ...
  {
   edit($("#hiddenid").val(),name,threshold)
  }
...
function editme(val)
{
  ...
  $("#threshold").val($("#threshold"+val).text())
...
function deleteme(val)
{
  ...
  $("#exampleModalLabel").text("Delete Threshold")
  $("#name").val($("#name"+val).text())
  $("#threshold").val($("#threshold"+val).text())
  $("#hiddenid").val(val)
}
function add(name,threshold)
{
  ...
  $.ajax({
     ...
     <td class="py-5 px-6" id=\"name'+data+'\">'+name+'</td>\
     <td class="py-5 px-6" id=\"threshold'+data+'\">'+threshold+'</td>\
     <td class="py-5 px-6">\
...
function edit(id,name,threshold)
{
  $.ajaxSetup({
    headers: {
      'X-CSRF-TOKEN': $("meta[name='csrf_token']").attr('content')
  ...
  $.ajax({
     type:'post',
     url:'editGroup',
     data: {name:name,threshold:threshold,id:id},
     success:function(data) {
     if(data=="")
     {
       $("#name"+id).text(name)
       $("#threshold"+id).text(threshold)
     }
     }
  });
}
